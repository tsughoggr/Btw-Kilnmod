...
public class VIPTileEntityUnfiredBrick extends TileEntity implements FCITileEntityDataPacketHandler
{
    ...
    {
	int bid = this.worldObj.getBlockId(this.xCoord, this.yCoord, this.zCoord);
	int[] rcp;
	rcp = VIPCraftingManagerBrickDry.getInstance().findMatchingRecipe(bid);
	if(rcp == null){
        ...
        VIPBlockBrickStackWithTileEntity var6 = (VIPBlockBrickStackWithTileEntity)Block.blocksList[worldObj.getBlockId(xCoord, yCoord, zCoord)];

        if (this.m_bIsCooking)
        {
            ++this.m_iCookCounter;

            if (this.m_iCookCounter >= cooktime)
            {
		if(rcp != null)
			worldObj.setBlockAndMetadata(xCoord, yCoord, zCoord, rcp[1], worldObj.getBlockMetadata(xCoord, yCoord, zCoord) & 12);
		else
			worldObj.setBlockToAir(xCoord, yCoord, zCoord);
                return;
            }
    ...
    private int ComputeCookLevel()
    {
        if (this.m_iCookCounter > 0)
        {
            int var1 = (int)((float)this.m_iCookCounter / 12000.0F * 3.0F) ;
            return MathHelper.clamp_int(var1, 0, 3);
        }
        else
        {
            return 0;
